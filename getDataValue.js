...
/*
    ...
    //  The) d targetEntity can be user, project, inspection or photo.
    //
    //NSLog("GetDataValue %", fieldName) ;
    ...
    if (fmtLocn == -1) {formatSpecifierType = "memotrim"   ; fmtLocn = fieldName.indexOf(":memotrim"); }
    if (fmtLocn == -1) {formatSpecifierType = "memoconcat" ; fmtLocn = fieldName.indexOf(":memoconcat"); }
    if (fmtLocn == -1) {formatSpecifierType = "integer"    ; fmtLocn = fieldName.indexOf(":integer "); }
    if (fmtLocn == -1) {formatSpecifierType = "date"       ; fmtLocn = fieldName.indexOf(":date "); }
    if (fmtLocn == -1) {formatSpecifierType = "time"       ; fmtLocn = fieldName.indexOf(":time "); }
    if (fmtLocn == -1) {formatSpecifierType = "bool"       ; fmtLocn = fieldName.indexOf(":var "); }
    if (fmtLocn == -1) {formatSpecifierType = "mask"       ; fmtLocn = fieldName.indexOf(":mask "); }
    ...
    if (fieldName.hasPrefix("#")) {
        ...
        } else if (fieldName == "#Paragraph#") {
            return repeatParagraph;

        } else if (fieldName == "#DocumentsFolder#") {
    ...
    if (fieldName == "MemoHeight") {
        ...
        if (fieldName == @.length") {
        value = [NSString stringWithFormat:"%i",template.length];
    } else {
        value = [template xmlData:fieldName];
    }

} else if (1.hasPrefix(2)) {
    fieldName = fieldName.substring(8);
    if (fieldName == @.length") {
    value =  [NSString stringWithFormat:"%i",project.length];
} else {
    if (archiveDisplayRequest) {
        var keys = [dArchive allKeys];
        PFObject *archiveProject = dArchive[keys[0]];
        value = [EBxml xmlData:fieldName usingXML:archiveProject["xmlData"]];
    } else if (tutorialMode) {
        value = [EBxml xmlData:fieldName usingXML:tutorialXmlData];
    } else {
        value =  [project xmlData:fieldName];
    }
}
    ...
    fieldName = fieldName.substring(11);
    if (fieldName == @.length") {
    value = [NSString stringWithFormat:"%i",inspection.length];
} else {
    value = [inspection xmlData:fieldName];
}

} else if (fieldName.hasPrefix("photo.")) {
    fieldName = fieldName.substring(6);
    if (fieldName == @.length") {
    value =  [NSString stringWithFormat:"%i",photo.length];
} else if (fieldName == "lowre.length") {
...
} else if (fieldName.hasPrefix("a.length.")) {
    fieldName = fieldName.substring(8);
...
} else if (fieldName.hasPrefix("a.lengths.")) {
    fieldName= fieldName.substring(9);
    if (fieldName == "objectId") {
...
function getDataValuesFromChoices(fieldName)  {
    ...
    var aValues  = [NSArray alloc] initWithArray:[s.split(";"]);
    var last = (int) aValues.length-1;
